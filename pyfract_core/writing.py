...
class ModuleWriter:
    ...
    def write(
        ...
        (output_dir / "__init__.py").write_text("# Generated by modularizer\n")
    ...
    def _sort_modules_for_validation(output_dir: Path, written_files: List[str]) -> List[str]:
        stems = {Path(filename).stem for filename in written_files}
        if len(stems) <= 1:
            return list(written_files)

        graph: Dict[str, Set[str]] = {Path(filename).stem: set() for filename in written_files}
        indegree: Dict[str, int] = {Path(filename).stem: 0 for filename in written_files}
        ...
        for filename in written_files:
            ...
            try:
                ...
                continue
            for match in rel_imp.finditer(code):
                dep = match.group(1)
        ...
        stem_to_file = {Path(filename).stem: filename for filename in written_files}
